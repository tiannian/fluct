use evm::{
    ...
    executor::stack::{MemoryStackAccount, StackState, StackSubstateMetadata},
    ExitError, Transfer,
};
use primitive_types::{H160, H256, U256};

use super::{CoreStackSubstate, State};

#[derive(Clone, Debug)]
pub struct CoreStackState<'backend, 'config, B> {
    backend: &'backend B,
    substate: CoreStackSubstate<'config>,
}

impl<'backend, 'config, B: Backend> Backend for CoreStackState<'backend, 'config, B> {
...
impl<'backend, 'config, B: Backend> StackState<'config> for CoreStackState<'backend, 'config, B> {
